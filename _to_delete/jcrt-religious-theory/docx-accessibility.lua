...
function Pandoc(doc)
  ...
  local published = text(meta.published)
  ...
  cover[#cover + 1] = paragraph("Religious Theory | An Editorial Review Blog")
  if published ~= "" then cover[#cover + 1] = paragraph(published) end
  ...
  if text(meta.url) ~= "" then
    cover[#cover + 1] = paragraph({
      pandoc.Link({pandoc.Str("Read this review on JCRT")}, text(meta.url)),
      pandoc.LineBreak(),
      pandoc.Str(text(meta.url))
    })
  end
  if text(meta.doi) ~= "" then
    cover[#cover + 1] = paragraph({pandoc.Str("DOI: "), pandoc.Link({pandoc.Str("https://doi.org/" .. text(meta.doi))}, "https://doi.org/" .. text(meta.doi))})
  end
  if abstract ~= "" then
